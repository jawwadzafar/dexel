...
// is added on top. Building test fixtures this way
...
func tr(keyDelta uint64, mouseActive bool, focus uint64) engine.TickResult {
...
// coinSum totals a CoinBreakdown's three buckets.
...
// the three buckets must sum to EXACTLY the DevCash actually earned today,
...
func TestCoinBreakdownSumsToEarnedDevCash(t *testing.T) {
	...
	for i := 0; i < 401; i++ {
		g.Tick(tr(15, false, 0))
	}
	completed := g.Tick(tr(0, false, 1))
	if !completed {
...
// (≈4%) — with no mouse activity at all. The 25-coin payout
...
func TestCoinAttributionIsProportionalToWorkShare(t *testing.T) {
	...
	if completed := g.Tick(tr(0, false, 1)); !completed {
		t.Fatal("expected sprint 0 to complete just past 50 work units")
	}

	want := CoinBreakdown{Keystrokes: 24, Mouse: 0, FocusSessions: 1}
...
func TestWorkAccrualGatedByStoreOpenLikeProgress(t *testing.T) {
	...
	want := CoinBreakdown{Keystrokes: 0, Mouse: 25, FocusSessions: 0}
	got := g.State().Stats.CoinsToday
	if got != want {
