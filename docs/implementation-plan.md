...
# v0.2 — "it looks like a game" (post-MVP)

v0.1 (M0-M5) proved the loop works. It did not look like the product: the
scene was three colored `Node` rectangles with no character, and a vision
model shown a screenshot called it *"placeholder programmer art … no
artistic style present."* v0.2 closes the gap between "the mechanics work"
and "a person would want this on their desktop".

## A1 — Real pixel art

- `docs/art-direction.md` is the spec: an 18-colour fixed palette, a
  15-sprite manifest with authored pixel sizes, character design rules, and
  the back-to-front layering order. Art that improvises colours or sizes is
  what makes a scene read as incoherent, so the manifest is normative.
- Art is **generated by committed code** (`tools/gen_assets.py`), not
  hand-drawn binaries. Every sprite is therefore reproducible, diffable, and
  tweakable by changing a number. `assets/*.png` are committed build
  outputs.
- **Exit:** all 15 sprites exist at their exact authored sizes, every opaque
  pixel is a palette colour, and re-running the generator is byte-identical.

## A2 — Sprite scene + character animation

- `companion/src/scene.rs` (`ScenePlugin`) draws the room as sprites in a
  320x200 room-space, upscaled x2 to the 640x400 window, with every prop
  position derived from the surface it rests on.
- The character animates from `Developer.mood`: a 5 fps two-frame typing
  loop while `Coding`, `dev_idle` while `Idle`, `dev_coffee` while
  `OnBreak`. The monitor swaps on/off with the same mood.
- **Two integration traps, both hit and both worth remembering:**
  1. `bevy_ui` renders *above* the 2D sprite layer. The old opaque
     full-window `Node` hid a perfectly working sprite scene completely.
     The UI root must stay transparent — it exists only to host the HUD.
  2. Bevy resolves `AssetPlugin.file_path` against the **running package's**
     manifest dir, so `cargo run -p companion` and `cargo run -p shotcap`
     look in two *different* wrong places. `configured_default_plugins()`
     pins one absolute path for the whole workspace, and is also where
     `ImagePlugin::default_nearest()` lives — without it every sprite is
     bilinearly blurred and the pixel-art look is lost.
- **Exit:** a vision model shown a screenshot describes a character at a
  desk in a pixel-art room, and does not use the words "placeholder" or
  "programmer art".

## A3 — Global activity (the premise, finally honoured)

v0.1 counted activity only while the **game window** was focused — so while
you were actually coding in your editor, nothing counted and the companion
never progressed. The core premise was inert in its real use case.

- `activity::GlobalInputProvider`, behind the `global-input` feature, reads
  `/dev/input/event*` via `evdev` on its own thread.
- `evdev`, not `rdev`, deliberately: `rdev`'s Linux backend is X11
  `XRecord`, which cannot see Wayland-native clients — i.e. it would be
  blind to exactly the editor we care about on a Wayland session.
- **Privacy is unchanged and still enforced:** counts and durations only. A
  keycode is read inside a one-line predicate and dropped as a `bool`; it
  never reaches a field, a log, or a return value. `EV_MSC` scancodes, which
  do carry key identity, are ignored outright.
- Degrades explicitly: without membership of the `input` group it returns
  `NoReadableInputDevices` and the game falls back to the focused-window
  provider with a warning, rather than panicking or silently doing nothing.
- **Exit:** typing in another application advances the progress bar while the
  game window is unfocused.

## A4 — HUD in the art-direction palette

The HUD was functional but cold — a blue-grey panel and a bright modern
green bar against a warm pixel-art room. Restyled to the palette, with a
wooden top lip so it reads as a shelf the room sits on, and shortened from
88px to 64px (88 was 22% of a 400px window).

## Verifying visual work at all

No fleet model can see. Two tools close that:

- `tools/shotcap` runs the real app and captures its own framebuffer via
  Bevy's `Screenshot` component. External screenshot tools were a dead end
  here — the X display is unauthorised from agent shells and none are
  installed.
- `scripts/visual-check.py` sends a capture to a vision-capable model and
  prints the description. Ask it to *describe and critique*, never a leading
  yes/no question. It correctly called the v0.1 build "placeholder
  programmer art", so it discriminates well enough to gate on.

A test that asserts "the HUD entity was spawned" passes happily while the
HUD is invisible off-screen — that exact bug survived five milestones and
three approving reviewers. Mechanical proof and visual proof are different
claims; a milestone with a visual criterion needs both.
